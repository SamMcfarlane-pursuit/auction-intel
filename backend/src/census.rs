...
static FIPS_TO_STATE: Lazy<HashMap<&str, &str>> =
    Lazy::new(|| STATE_FIPS.iter().map(|(k, v)| (*v, *k)).collect());
...
pub async fn fetch_all_counties(api_key: Option<&str>) -> Result<Vec<CountyCensusData>, String> {
    ...
    // Skip header row
    let counties: Vec<CountyCensusData> = data
        .iter()
        .skip(1)
        .filter_map(|row| {
            if row.len() < 8 {
...
pub async fn fetch_state_counties(
    state_abbr: &str,
    api_key: Option<&str>,
) -> Result<Vec<CountyCensusData>, String> {
...
// (county_stats function removed as it was unused)
