...
// State name mappings
// (Removed unused STATE_NAMES static)
...
static FORECLOSURE_CACHE: Lazy<Arc<ForeclosureCache>> =
    Lazy::new(|| Arc::new(ForeclosureCache::new()));
...
pub fn get_national_trends() -> NationalTrends {
    ...
    let avg_rate: f64 =
        stats.values().map(|s| s.foreclosure_rate).sum::<f64>() / stats.len() as f64;
