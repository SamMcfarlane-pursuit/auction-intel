...
function getStateColor(isLien) {
    return isLien ? '#4F46E5' : '#0D9488'; // indigo-600 for Lien, teal-600 for Deed
}

function getStateColorLight(isLien) {
    return isLien ? '#6366F1' : '#14B8A6'; // indigo-500 hover, teal-500 hover
}
...
function TooltipCard({ data }) {
    ...
    return (
        ...
        >
            <div className="flex items-center gap-3 bg-surface/95 backdrop-blur-xl border border-slate-300/80 shadow-none rounded-sm px-4 py-2 whitespace-nowrap overflow-hidden">
                {/* Status Beacon */}
                <div className={`w-2 h-2 rounded-sm ${isLien ? 'bg-indigo-500 shadow-[0_0_8px_rgba(59,130,246,0.8)] animate-none' : 'bg-slate-400 shadow-[0_0_8px_rgba(100,116,139,0.8)]'}`} />
                ...
                {info ? (
                    <div className="flex items-center gap-3 ml-1">
                        <div className="w-px h-3 bg-panel-2"></div>
                        <div className="flex items-center gap-1.5">
                            <span className="text-[8px] text-slate-500 font-bold uppercase tracking-widest">Yield</span>
                            <span className="text-[10px] font-semibold text-emerald-600 font-mono tracking-tighter">{info.interestRate}</span>
                        </div>
                        <div className="w-px h-3 bg-panel-2"></div>
                        <div className="flex items-center gap-1.5">
                        ...
                        <div className="flex items-center gap-1.5">
                            ...
                            <span className="text-[9px] font-bold text-slate-700 truncate max-w-[80px] text-right">{info.biddingType || 'Varies'}</span>
                        </div>
                    </div>
                ) : (
                    <div className="flex items-center gap-3 ml-1">
    ...
    return (
        <div
            style={{
                width: '100%',
                height,
                background: 'radial-gradient(ellipse at 50% 50%, #FFFFFF 0%, #F1EEE8 100%)',
                position: 'relative',
                borderRadius: '1rem',
                overflow: 'hidden',
        ...
        >
            ...
            <div className="absolute top-6 left-6 pointer-events-none">
                <div className="flex items-center gap-2 mb-1">
                    <div className="w-1.5 h-1.5 rounded-sm bg-indigo-500 shadow-[0_0_8px_#3b82f6]" />
                    <span className="text-[9px] text-slate-600 font-bold uppercase tracking-[0.3em]">Interactive Analysis</span>
                </div>
