...
export default function UnderwritingPanel({ property, onCalculate }) {
    ...
    return (
        <div className="flex flex-col h-full bg-surface border-l border-slate-300 shadow-none animate-in slide-in-from-right duration-300">
            <div className="p-6 border-b border-slate-300 bg-slate-100/60">
                ...
                <p className="text-slate-600 text-xs mt-1 uppercase tracking-widest font-bold">Model 70.3 Premium</p>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
                ...
                {results && (
                    <div className="space-y-4 pt-4 border-t border-slate-200">
                        <div className="bg-surface /20 /5 rounded-md p-5 border border-emerald-500/20">
                            <div className="text-[10px] text-emerald-600 font-semibold uppercase tracking-widest mb-1">Max Allowable Bid</div>
                            <div className="text-xl font-mono font-semibold text-white">${Math.round(results.max_allowed_bid).toLocaleString()}</div>
                            <div className="mt-2 h-1.5 w-full bg-panel rounded-sm overflow-hidden">
                                <div 
 ...
 clearingBounds.safety_sentiment === 'Favorable Buyer Market' ? 'bg-emerald-100 text-emerald-600' :
 clearingBounds.safety_sentiment === 'Aggressive Market' ? 'bg-amber-100 text-amber-400' :
 'bg-indigo-500/20 text-indigo-500'
                                ...
                                <div className="flex justify-between text-[10px] font-bold text-slate-600 mb-2">
                                    ...
                                    <span className="text-rose-600">Danger: {'>'}$ {Math.round(clearingBounds.danger_zone_threshold).toLocaleString()}</span>
                                </div>
                                
                                {/* Trinary Range Slider Bar */}
                                <div className="h-2 w-full bg-panel rounded-sm overflow-hidden flex">
                                    ...
                                    <div className="bg-rose-500 h-full transition-all" style={{ flex: 1 }} />
                                </div>
                            </div>
                        )}
                        ...
                        <div className="bg-slate-100/60 rounded-sm p-4 border border-slate-300">
                            ...
                            <div className="flex justify-between items-center">
                                <span className="text-xs text-slate-600">Equity at Max Bid</span>
                                <span className="text-sm font-bold text-emerald-600">${Math.round(results.equity_position).toLocaleString()}</span>
                            </div>
                        </div>
                    </div>
                )}
...
function InputGroup({ label, value, onChange, prefix, color = "text-white" }) {
    return (
        <div className="space-y-1.5">
            <label className="text-[10px] font-bold text-slate-600 uppercase tracking-tighter ml-1">{label}</label>
            <div className="relative group">
                ...
                <input 
                    ...
                    className={`w-full bg-panel border border-slate-300 rounded-sm py-3 pl-8 pr-4 text-sm font-bold ${color} outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500/20 group-hover:border-slate-500 transition-all`}
                />
            </div>
        </div>
    );
...
function StatBox({ label, value, sub }) {
    return (
        <div className="bg-panel/30 border border-slate-300 rounded-sm p-4">
