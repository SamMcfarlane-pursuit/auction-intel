...
export default function MapToolbar({ 
    ...
    return (
        <div className="flex items-center gap-2">
            ...
            <div className="bg-slate-900/70 backdrop-blur-2xl border border-white/10 rounded-sm p-1 flex gap-0.5 shadow-none ring-1 ring-white/5">
                {modes.map((mode) => (
                    <button
                        key={mode.id}
 ...
 currentMode === mode.id 
 ? 'bg-slate-950 text-slate-100 shadow-none' 
 : 'text-slate-400 hover:text-white hover:bg-slate-950/5'
 }`}
                    >
                        <span className="text-sm">{mode.icon}</span>
                        <span className={`text-[10px] font-semibold uppercase tracking-widest ${
 currentMode === mode.id ? 'text-slate-100' : ''
 }`}>
                            {mode.label}
                        </span>
                    </button>
 ...
 : 'bg-slate-900/70 text-slate-400 border-white/10 hover:text-white hover:bg-slate-950/5'
 }`}
